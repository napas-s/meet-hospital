...
<?php
	...
	//ข้อมูลส่วนบุคคล
	$userId 	    		    = $_POST['userId'];
	$user_iden13 	    		= isset($row['iden13_hd']) ? $_POST['iden13_hd'] : '';
	$user_prename 	    		= isset($row['prename']) ? $_POST['prename'] : '';
	$user_prenameOthers 	    = isset($row['prenameOther']) ? $_POST['prenameOther'] : '';
	$user_fname 	    		= isset($row['fname']) ? $_POST['fname'] : '';
	$user_lname 	    		= isset($row['lname']) ? $_POST['lname'] : '';
	$user_sex 	    			= isset($row['sex']) ? $_POST['sex'] : '';
	$user_birthday 	    		= date("Y-m-d", strtotime($_POST['birthday']));
	$user_nation 	    		= isset($row['nation']) ? $_POST['nation'] : '';
	$user_nationOther 	    	= isset($row['nationOther']) ? $_POST['nationOther'] : '';
	$user_country 	    		= isset($row['user_birthday']) ? $_POST['country'] : '';
	$user_countryOther 	    	= isset($row['countryOther']) ? $_POST['countryOther'] : '';
	$user_province_birth 	    = isset($row['province_birth']) ? $_POST['province_birth'] : '';
	$user_provinceOther 	    = isset($row['provinceOther']) ? $_POST['provinceOther'] : '';
	$user_job 	    			= isset($row['job']) ? $_POST['job'] : '';
	$user_faith 	    		= isset($row['faith']) ? $_POST['faith'] : '';
	$user_faithOther 	    	= isset($row['faithOther']) ? $_POST['faithOther'] : '';
	$user_education 	    	= isset($row['education']) ? $_POST['education'] : '';
	$user_marry_status 	    	= isset($row['marry_status']) ? $_POST['marry_status'] : '';
	$user_blood 	    		= isset($row['blood']) ? $_POST['blood'] : '';
	$user_allergy 	    		= isset($row['allergy']) ? $_POST['allergy'] : '';
	$user_allergyOther 	    	= isset($row['allergyOther']) ? $_POST['allergyOther'] : '';
	$user_father_name 	    	= isset($row['father_name']) ? $_POST['father_name'] : '';
	$user_mother_name 	    	= isset($row['mother_name']) ? $_POST['mother_name'] : '';
